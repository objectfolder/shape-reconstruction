...
def load_mesh_vision(obj_name, args, use_touch = False):
	# load obj file
	...
	adj_info = adj_init(verts, faces, args, use_touch)
...
def adj_init(verts, faces, args, use_touch):
	...
	if use_touch:
		...
		adj, faces = adj_fuse_touch(verts, faces, adj, args, use_touch)
...
def adj_fuse_touch(verts, faces, adj, args, use_touch=False):
	...
	sheet = import_obj(osp.join(osp.dirname(osp.abspath(__file__)), 'initial_sheet.obj'))
	...
	central_point = 4
	central_points = [central_point + (i * sheet_adj.shape[0]) + adj.shape[0] for i in range(args.touch_num)]

	# define and fill new adjacency matrix with vision and touch charts
	new_dim = adj.shape[0] + (args.touch_num * sheet_adj.shape[0])
	new_adj = torch.zeros((new_dim, new_dim)).cuda()
	new_adj[: adj.shape[0], :adj.shape[0]] = adj.clone()
	for i in range(args.touch_num):
